...
function createDecipheriv(key) {
    const keyAndIv = crypto.createHash('md5')
    ...
    return crypto.createDecipheriv('aes-128-cbc', Buffer.from(secretKey, 'utf-8'), Buffer.from(iv, 'utf-8'));
}
function createCipher(key) {
    ...
    const secretKey = keyAndIv.slice(0, halfKeyLen);
    const iv = keyAndIv.slice(halfKeyLen);
    return crypto.createCipheriv('aes-128-cbc', Buffer.from(secretKey, 'utf-8'), Buffer.from(iv, 'utf-8'));
}
...
export function encrypt(clientKey, payload) {
...
export function nextClientKey(clientKey) {
